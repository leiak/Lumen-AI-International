...
public class SysDictServiceImpl implements SysDictService {
    ...
    /**
     * 按字典 code 解析 id 后取所有字典项，按 sort_order 升序返回。
     *
     * <p>字典 code 是业务侧稳定的标识（{@code sys_dict.code} 唯一索引），前端
     * 用它去拉字典项：{@code useDict} hook 用它构造 valueEnum，Dict 页字典项
     * 管理 Drawer 用它渲染子表并按 id 编辑/删除。先按 code 查出 id、再按 id
     * 拉字典项，避免让前端在管理页主动传入主键。
     *
     * <p>code 不存在时返回空列表（不是 404）—— 字典项管理页可能还没创建，前端
     * 会收到空列表后降级展示。
     */
    @Override
    public List<SysDictItem> listItemsByDictCodeEntity(String code) {
